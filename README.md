...
我很喜欢 Codex 桌面端那种高密度、以项目为中心的 UI 和工作流，所以做了一个独立的 DeepX：外观和交互方向参考这种桌面 agent 体验，但模型接入、配置目录、后端 core 和发行包都是 DeepX 自己维护。

DeepX 不是 DeepSeek 官方项目，也不是 OpenAI/Codex 官方项目；本项目与 DeepSeek 官方、OpenAI 官方均无隶属、背书或合作关系。

## 特性

- 自带 DeepSeek 优先的多服务商配置：DeepSeek、MiMo、GLM、Qwen、Kimi、StepFun、MiniMax，以及自定义 OpenAI-compatible provider。
- 用户本机 API key 存在 `data/secrets.local.json`，发行包默认不包含任何密钥。
- Portable Windows 发行版：应用运行态、配置、会话、日志、插件、skills、缓存指标都放在 DeepX 目录内。
- 项目式工作区：每个项目保留独立上下文和会话入口，默认只读文件 Agent。
- 权限模式可选：默认权限、自动审查、完全访问权限、自定义配置。首版默认只读，写文件和 shell 执行需要确认 UI 成熟后再开放。
...
- [DeepSeek-GUI](https://github.com/XingYu-Zhong/DeepSeek-GUI)：参考了社区 DeepSeek 桌面项目的公开 README/桌面应用组织方式。
