...
export function exportPapers(papers: CatalogPaper[], format: "csv" | "json" | "bib") {
  ...
  if (format === "csv") {
    const rows = [["标题", "年份", "作者", "机构数", "任务", "贡献类型", "Venue", "CCF", "审核状态", "链接"], ...papers.map((paper) => [paper.title, paper.year, paper.authors.map((x) => x.name).join("; "), paper.institution_ids.length, paper.task_tags.join(";"), paper.contribution_type, paper.venue?.name ?? "", paper.venue?.rankings.find((x) => x.system === "CCF")?.level ?? "", paper.review_status, paper.primary_url])];
