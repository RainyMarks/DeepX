...
OUT_OF_SCOPE = re.compile(
    ...
    r"音频|视频|语音|文本生成|语言模型|纯\s*ocr|相机型号|番茄病害",
    re.IGNORECASE,
)
...
def bulk_review() -> dict[str, int]:
    ...
    if decision_path.exists():
        for item in iter_jsonl(decision_path):
    ...
    for paper in catalog:
        if paper.get("review_status") != "auto" or paper["id"] in decisions:
            ...
            continue
        evidence_text = " ".join(
            [paper.get("title", ""), paper.get("abstract_excerpt", ""), " ".join(paper.get("task_tags", []))]
        )
        has_source = bool(paper.get("primary_url") and paper.get("sources"))
        if not has_source or OUT_OF_SCOPE.search(evidence_text):
            status = "rejected"
            note = "快速批量核验：缺少稳定来源或命中明确非图像取证主题，予以排除。"
        else:
            status = "verified"
            note = "快速批量核验：具有稳定来源，且任务标签与图像取证范围一致。"
        decisions[paper["id"]] = {
            "paper_id": paper["id"],
            "status": status,
